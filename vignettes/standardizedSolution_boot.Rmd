...
This vignette is a quick guide
to use `standardizedSolution_boot()`
in the package [`semboottools`](https://yangzhen1999.github.io/semboottools/),
described in @yang_forming_2026,
to form bootstrap confidence intervals
for the standardized solution in a model
fitted by `lavaan`.
...
The function `standardizedSolution_boot()`
can be used directly when bootstrap
standard errors and confidence intervals
are requested when fitting the model
(using `se = "boot"`):
...
If bootstrap standard errors are not
requested when fitting the model,
call `store_boot()` first. It does
the following:

- Does bootstrapping using `bootstrapLavaan()`.

- Stores the bootstrap estimates in the
  object and returns it. This object can
  be used as an usual `lavaan` output
  object.

This method is useful when both the default
standard errors and *p*-values, such as
those by maximum likelihood (ML), and the
bootstrap standard errors and *p*-values
are desired.

The function `standardizedSolution_boot()`
can then be used directly on the output
of `store_boot()`.
...
Additional options to customize the
output of `standardizedSolution_boot()`.
