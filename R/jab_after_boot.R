...
.make_jab_plot_gg <- function(
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(x = .data$idx, y = .data$y)) +
    ...
    ggplot2::geom_hline(data = line_df, ggplot2::aes(yintercept = .data$y, linetype = .data$lab),
    ...
    # 仅用 size 强调点大小，不单独出图例
    ggplot2::scale_size_manual(values = c("Other cases" = 1.6, "Top influential cases" = 2.6), guide = "none") +
    ggplot2::scale_linetype_manual(values = c("Full-sample bootstrap mean" = "solid")) +
    ggplot2::labs(
    ...
    ggplot2::guides(
      ...
      color = ggplot2::guide_legend(
        ...
        keyheight = ggplot2::unit(6, "mm"),
        keywidth  = ggplot2::unit(10, "mm")
      ),
      # 线型图例：只有线
